...
class ServerResponse(BaseModel):
    ...
    def translate_file_path(cls, data: Dict):
        ...
        """
        if 'path' in data:
            data['local_path'] = cls.__translate_filepath_to_local(
                data['path'],
                data['mount_info'].local_mount_location,
                data['mount_info'].server_mount_location
            )

            data['remote_path'] = data['path']
    ...
    def __translate_filepath_to_local(path: Path, local_mount_location: Path, server_mount_location: Path) -> Path:
        ...
        normalized_server_mount = str(server_mount_location).replace('/', os.sep).replace('\\', os.sep)
        normalized_local_mount = str(local_mount_location).replace('/', os.sep).replace('\\', os.sep)
